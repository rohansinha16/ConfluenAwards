...
function bubbleChart() {
	...
	var smallData = [29994, 25000, 24427, 24963,24942,15000,29804,29861,22982,28033,29425,25797,30000,25000,7500,25000,24903,24996,24994,25000,25000,24909,24999,24974,24882,8000,18796,20000,20000,10000,15800,15000,14338,15000,14767,12360,13817,15000]
	 ...
	 */
	 var chart = function chart(selector, rawData) {
		...
		hugeSize = radiusScale(+7600000) * width/screen.availWidth * 2;
	 ...
	 function moveToCenter(alpha) {
	 	return function (d) {
	 		if(d.id < 18){
	 			...
	 			d.x = d.x + (center.x - d.x) * damper * alpha;
	 			d.y = d.y + ((center.y - height) - d.y) * damper * alpha;
	 		}
	 	};
	 }
	...
	function moveBigBlue(alpha){
		return function (d) {
			if(d.id < 18 && d.grant === 'Faculty Collaboration Grant'){
		 		d.x = d.x + (center.x - d.x) * damper * alpha;
	 			d.y = d.y + (1.5 * height - d.y) * damper * alpha;
		 	}
		 	else if(d.id >= 18){
	 			d.x = d.x + (center.x - d.x) * damper * alpha;
	 			d.y = d.y + (center.y - d.y) * damper * alpha;
	...
	function keepSix(alpha){
	 	return function (d) {
	 		if(d.id >= 18 && d.id <=23){
	 			...
	 			d.y = d.y + ((Math.random() * (2 * height) - height/2) - d.y) * damper * alpha;
	 		}
	 	};
	}
